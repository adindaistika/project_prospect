...
import { getContact, getContactByCategory, getContactById } from "../../../store/reducers/contact/contact.action";
import ReactPaginate from "react-paginate";

export default function prospect() {
  const dispatch = useDispatch();
  const { data_contactcategory } = useSelector((state) => state.contactcategory);
  ...
  return (
    <main className=" min-h-screen">
      <div className="flex gap-[10px]">
        <select onChange={(e) => selectCategory(e.target.value)} className="outline-none focus:outline-none select w-full max-w-xs rounded-md shadow-md text-[#9ca3af]">
          <option selected value={0} key={0}>All Category</option>
          {data_contactcategory.length > 0 && data_contactcategory.map((item, k) => 
            <option value={item.id} key={k+1}>{item.category_name}</option>
          )}
        </select>
      ...
      <div className="grid md:grid-cols-3 mt-5 gap-3">
        ...
        : 
        <h5 className="font-semibold">Data kosong...</h5>
        }
        <ModalProspect />
      </div>
      <div>
